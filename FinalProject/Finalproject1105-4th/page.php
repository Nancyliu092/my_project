...
if ($result->num_rows > 0) {
    ...
    //  $zipData = $row['picture']; // 取得壓縮檔資料

    //  // 儲存臨時檔案
    //  $zipFile = 'temp.zip';
    //  file_put_contents($zipFile, $zipData); // 將資料寫入臨時 zip 檔案
 
    //  // 解壓縮檔案
    //  $zip = new ZipArchive;

    //  if ($zip->open($zipFile) === TRUE) {
    //     // 假設壓縮檔案內只有一個圖片文件，獲取檔案名稱
    //     $imageFileName = $zip->getNameIndex(0); // 取壓縮包內的第一個文件
    ...
    //     // 顯示圖片
    //     $imageSrc = $extractPath . $imageFileName;
    
        echo '<div class="flex-container1-1">';
        echo '    <div class="flex-container1-1-2">';
        echo '        <div class="shortcut">';
        echo '            <div id="Home" class="listing"><a href="finalproject.php">back</a></div>';
        ...
        echo '
            ...
            echo '
                <li class="card">
                    <div class="img">
                        <a href="page.php?id=' . htmlspecialchars($row_other["id"]) . '">
